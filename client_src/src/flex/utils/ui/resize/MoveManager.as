package flex.utils.ui.resize
{
	...
	public class MoveManager
	{
		...
		private var parentQuadrant:DeskQuadrant;
		...
		public function addMoveSupport(moveComponent:UIComponent, dragComponent:UIComponent = null, 
			...
			var parentGroup:Group = moveComponent.parent as Group; 
			this.parentQuadrant = parentGroup.parentDocument as DeskQuadrant;
...
//		}
		...
		private function dragComponentMouseDown(event:MouseEvent):void {
			...
			if (bringToFrontOnMove && moveComponent.parent) {
				var parentGroup:Group = moveComponent.parent as Group;
				var index:int = parentGroup.getElementIndex(moveComponent);
				var last:int = parentGroup.numElements - 1;
				if (index != last) {
					parentGroup.setElementIndex(moveComponent, last);
				}
			}
		...
		private function dragComponentMove(event:MouseEvent):void {
			...
			if(event.target is Group) {
				var groupTarget:Group = event.target as Group;
				if ((groupTarget.parentDocument is DeskQuadrant) && 
					(groupTarget.parentDocument != parentQuadrant)) {
					removeMoveCursor();
					
					var ds:DragSource = new DragSource();
					var note:StickyNote = moveComponent as StickyNote;
					ds.addData(note, 'noteItself');
					
					DragManager.doDrag(note, ds, event);				
				} else {
